...
function server(msg) {
    return new Promise(async (resolve) => {
        try {
            ...
            if (matchResult) {
                url = `https://${matchResult[1]}.push.ft07.com/send/${sckey}.send?tags=OneSign`;
            } else {
                url = `https://sctapi.ftqq.com/${sckey}.send`;
            }
            let data = `title=${encodeURI("OneSign每日任务已完成")}&desp=${encodeURI(msg.replace(/\n/g, "\n\n"))}`
